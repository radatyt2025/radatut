...
import { updateTaskStatus, addTaskComment } from '@/lib/action/tasks';
...
export default function TasksClient({ initialTasks, users }: TasksClientProps) {
  ...
  const handleStatusChange = (taskId: string, newStatus: TaskModel['status']) => {
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (canEditStatus && !isPending) {
                            setOpenStatusDropdown(isDropdownOpen ? null : task.id);
                          }
                        }}
                      >
...
const ChevronDownIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#002aff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
